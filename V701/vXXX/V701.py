...
def Dx(x, m, dn, dm):
    return np.sqrt(dn ** 2 + x ** 2 * dm ** 2) / m
...
ax.plot(
    x_p,
    x_p * params1[0] + params1[1],
    "b-",
    label=f"Lineare Regression:E={params1[0]:.3}"
    r"$\unit{\mega\electronvolt\per\milli\bar} \cdot$"
    f"p+{params1[1]:.3}"
    r"$\unit{\mega\electronvolt}$",
)
ax.set_xlabel(r"p/$\unit{\milli\bar}$")
ax.set_ylabel(r"E/$\unit{\mega\electronvolt}$")
ax.legend(loc="best")
print(
    f"Der Energieverlust pro Meter beträgt {params1[0]*1013/(x0*100):.3}"
    r"$\unit{\mega\electronvolt\per\centi\meter}"
)
...
plt.plot(
    ...
    r"$\frac{\text{Detektionen}}{\text{2 Minuten}}$"
    r"$=\num{-9.13e6}$"
    r"$\unit{\per\meter} \cdot$"
    r"x$+\num{2.37e5}$",
)
y2 = y_p * Z[0] / 2
plt.plot(x_p, y2, "g-", label=f"Halbe Zählrate")
SP = (Z[0] / 2 - params2[1]) / params2[0]
dSP = Dx(SP, params2[0], errors2[1], errors2[0])
print(f"Schnittpunkt={SP:.3}({dSP:.5})")
