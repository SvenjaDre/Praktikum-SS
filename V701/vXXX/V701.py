...
fig,ax=plt.subplots(layout="constrained")
ax.hist(np_array3[:], 11,range=(3900,4600),label="Datensatz")
ax2 = ax.twinx()
...
k=np.linspace(390, 460,11)
y = poisson.pmf(k, mu=Mittel)

fig,ax=plt.subplots(layout="constrained")
ax.hist(np_array3[:]/10, 11,range=(390,460),label="Datensatz")
ax2 = ax.twinx()
ax2.plot(k,y,"ro",label="Poissonverteilung")
ax2.set_ylabel(r"Wahrscheinlichkeit")

ax.set_xlabel(r"Anzahl an Detektionen in 1s")
ax.set_ylabel(r"Häufigkeit eines Ereignisses")
